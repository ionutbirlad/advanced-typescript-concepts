...
if (!recordBtn) {
  ...
  recordBtn.addEventListener('click', async () => {
    try {
      if (!moduleCache.Analytics) moduleCache.Analytics = await import('./analytics');
...
if (!loadProfileBtn) {
  ...
  loadProfileBtn.addEventListener('click', async () => {
    try {
      if (!moduleCache.UserManagement) moduleCache.UserManagement = await import('./userManagement');
